        ...
        public static int Solution()
        {
            ...
            for(int r = 0; r < rows; r++)
            {
                for(int c = 0; c < columns; c++)
                {
                    if(c < columns - 3)
                    {
                        product = Math.Max(product, grid[r, c] * grid[r, c + 1] * grid[r, c + 2] * grid[r, c + 3]);
                    }
                    if(r < rows - 3)
                    {
                        product = Math.Max(product, grid[r, c] * grid[r + 1, c] * grid[r + 2, c] * grid[r + 3, c]);

                        if (c > 2)
                        {
                            product = Math.Max(product, grid[r, c] * grid[r + 1, c - 1] * grid[r + 2, c - 2] * grid[r + 3, c - 3]);
                        }
